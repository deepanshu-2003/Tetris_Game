...
class LShape2():
    def __init__(self,win,min,max):
        self.window,self.MIN,self.MAX,self.t,self.color = win,min,max,time,"#8E18C5"
        # -----------forms-------------------------
        self.forms = {
                'l': [(self.MAX//2,0),((self.MAX//2)-30,0),((self.MAX//2)+30,0),((self.MAX//2)+30,-30)],
                'l_reverse': [(self.MAX//2,0),(self.MAX//2,30),(self.MAX//2,-30),((self.MAX//2)-30,-30)],
                'l_left' :[(self.MAX//2,0),((self.MAX//2)-30,0),((self.MAX//2)+30,0),((self.MAX//2)-30,30)],
                'l_right' :[(self.MAX//2,0),(self.MAX//2,30),(self.MAX//2,-30),((self.MAX//2)+30,30)],
        }
    ...
    def move_left(self,blocks):
        if(self.rec1.left-30>self.MIN and self.rec2.left-30>self.MIN and self.rec4.left-30>self.MIN):
            self.rec1.x-=30
            self.rec2.x-=30
            self.rec3.x-=30
            self.rec4.x-=30
    ...
    def move_right(self,blocks):
        if(self.rec3.right+30<self.MAX and self.rec4.right+30<self.MAX):
            self.rec1.x+=30
            self.rec2.x+=30
            self.rec3.x+=30
            self.rec4.x+=30
    ...
    def change(self,blocks):
        ...
        elif(self.form == "l_reverse"):
            previous = [[self.rec1.x,self.rec1.y],[self.rec2.x,self.rec2.y],[self.rec3.x,self.rec3.y],[self.rec4.x,self.rec4.y],self.form]
            switch_it("l_left")
            if collide_walls():
                set_previous(previous)
        elif(self.form == "l_left"):
            previous = [[self.rec1.x,self.rec1.y],[self.rec2.x,self.rec2.y],[self.rec3.x,self.rec3.y],[self.rec4.x,self.rec4.y],self.form]
            switch_it("l_right")
            if collide_walls():
                set_previous(previous)
        elif(self.form == "l_right"):
            previous = [[self.rec1.x,self.rec1.y],[self.rec2.x,self.rec2.y],[self.rec3.x,self.rec3.y],[self.rec4.x,self.rec4.y],self.form]
            switch_it("l")
            if collide_walls():
                set_previous(previous)
