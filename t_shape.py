...
class TShape():
    def __init__(self,win,min,max):
        self.window,self.MIN,self.MAX,self.t,self.color = win,min,max,time,(168,15,67)
        # -----------forms-------------------------
        self.forms = {
                't': [(self.MAX//2,0),((self.MAX//2)-30,0),((self.MAX//2)+30,0),(self.MAX//2,-30)],
                't_reverse': [(self.MAX//2,0),((self.MAX//2)-30,0),((self.MAX//2)+30,0),(self.MAX//2,30)],
                't_left' :[(self.MAX//2,0),((self.MAX//2)-30,0),(self.MAX//2,30),(self.MAX//2,-30)],
                't_right' :[(self.MAX//2,0),((self.MAX//2)+30,0),(self.MAX//2,30),(self.MAX//2,-30)],
        }
    ...
    def draw_demo(self,x,y):
        ...
        elif self.next_form=="t_reverse":
            pygame.draw.rect(self.window,self.color,pygame.Rect(x,y,30,30))
            pygame.draw.rect(self.window,self.color,pygame.Rect(x-30,y,30,30))
            pygame.draw.rect(self.window,self.color,pygame.Rect(x+30,y,30,30))
            pygame.draw.rect(self.window,self.color,pygame.Rect(x,y+30,30,30))
        elif self.next_form =="t_left":
            pygame.draw.rect(self.window,self.color,pygame.Rect(x,y,30,30))
            pygame.draw.rect(self.window,self.color,pygame.Rect(x-30,y,30,30))
            pygame.draw.rect(self.window,self.color,pygame.Rect(x,y+30,30,30))
            pygame.draw.rect(self.window,self.color,pygame.Rect(x,y-30,30,30))
    ...
    def move_left(self,blocks):
        if(self.rec1.left-30>self.MIN and self.rec2.left-30>self.MIN):
            self.rec1.x-=30
            self.rec2.x-=30
            self.rec3.x-=30
            self.rec4.x-=30
    ...
    def move_right(self,blocks):
        if(self.rec2.right+30<self.MAX and self.rec3.right+30<self.MAX and self.rec1.right+30<self.MAX):
            self.rec1.x+=30
            self.rec2.x+=30
            self.rec3.x+=30
            self.rec4.x+=30
    ...
    def change(self,blocks):
        ...
        elif(self.form == "t_reverse"):
            previous = [[self.rec1.x,self.rec1.y],[self.rec2.x,self.rec2.y],[self.rec3.x,self.rec3.y],[self.rec4.x,self.rec4.y],self.form]
            switch_it("t_left")
            if collide_walls():
                set_previous(previous)
        elif(self.form == "t_left"):
            previous = [[self.rec1.x,self.rec1.y],[self.rec2.x,self.rec2.y],[self.rec3.x,self.rec3.y],[self.rec4.x,self.rec4.y],self.form]
            switch_it("t_right")
            if collide_walls():
                set_previous(previous)
        elif(self.form == "t_right"):
            previous = [[self.rec1.x,self.rec1.y],[self.rec2.x,self.rec2.y],[self.rec3.x,self.rec3.y],[self.rec4.x,self.rec4.y],self.form]
            switch_it("t")
            if collide_walls():
                set_previous(previous)
